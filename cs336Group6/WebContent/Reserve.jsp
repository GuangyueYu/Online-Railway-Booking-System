...
<body>
	<%
		try {
			...
			String query = "SELECT * FROM Have_Reservation ORDER BY reserve_num";
			//Get parameters from the HTML form
			PreparedStatement ps = con.prepareStatement(query);
			ResultSet rs = ps.executeQuery();
			
			while(rs.next()) {
				count = Integer.parseInt(rs.getString("reserve_num"));
			}
			count++;
			stmt = con.createStatement();
			String reserve_num = String.valueOf(count);
...
<form method="get" action="home.jsp">
	<br>
		...
		<%
			response.sendRedirect("home.jsp?username="+request.getParameter("username"));
		%>
	<br />
</form>
</body>
